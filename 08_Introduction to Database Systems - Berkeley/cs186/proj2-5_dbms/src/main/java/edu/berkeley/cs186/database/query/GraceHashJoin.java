...
public class GraceHashJoin {
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        // Our hash table to build on. The list contains all the records of build relation.
    ...
    public static Pair<List<Record>, List<Record>> getBreakNHJInputs() {
        ...
        // A partition must be greater than 4 buffers (B-2), to break naive, while each buffer contains 976 records per page,
        // rightRecords can be any size.
        int breakPointNaive = 976*21;
        for (int i = 0; i < breakPointNaive; ++i) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        int breakPointGHJ = 4 * 976 + 1;
        for (int i = 0; i < breakPointGHJ; ++i) {
