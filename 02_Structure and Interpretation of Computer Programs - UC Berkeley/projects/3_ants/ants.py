...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_point, distance = self.place, 0
        while current_point.name != beehive.name:
            if distance >= self.min_range and distance <= self.max_range:
                bee = random_or_none(current_point.bees)    
                if bee:
                    return bee
            distance += 1
            current_point = current_point.entrance
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # Attack bees in place with armor amount
        for bee in list(self.place.bees):
            Insect.reduce_armor(bee, amount)
        
        self.armor -= amount
        if self.armor <= 0:
            # If dead, attack by bonus amount
            for bee in list(self.place.bees):
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each round, but blocks the way for bees"""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        # set the contained_ant to ant argument
    ...
    def action(self, colony):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        # Perform action of the contained ant
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.true_queen:
            # Throw leave at nearest bee
            ThrowerAnt.action(self, colony)

            # Double damage of ants behind her in tunnel
            current_point = self.place.exit # Skip place of queen
            while current_point:
                ant = current_point.ant
                if ant:
                    if ant not in QueenAnt.damage_doubled:
                        ant.damage = ant.damage * 2
                        QueenAnt.damage_doubled[ant] = True
                    # If container, double damage of inner ant
                    if current_point.ant.is_container and ant.contained_ant and ant.contained_ant not in QueenAnt.damage_doubled:
                        ant.contained_ant.damage = ant.contained_ant.damage * 2
                        QueenAnt.damage_doubled[ant.contained_ant] = True
                current_point = current_point.exit
        else:
