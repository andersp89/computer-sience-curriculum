...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set, QueryOperator> pass1TableAndAccessOperator = new HashMap<>();
        QueryOperator minCostQO = null;
        for (String table : aliases.keySet()) {
            minCostQO = minCostSingleAccess(table);
            Set<String> tableName = new HashSet<>();
            tableName.add(table);
            pass1TableAndAccessOperator.put(tableName, minCostQO);
        }
        ...
        // been joined.
        Map<Set, QueryOperator> prevMap = new HashMap<>(pass1TableAndAccessOperator);
        while (prevMap.size() > 1) {
            prevMap = minCostJoins(prevMap, pass1TableAndAccessOperator);
        }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        // Calculate IO cost of any indices on any colum of the table
        // that we have an selection predicate on
        // first, get all select predicates with index on column
        List<Integer> selectWithIndex = getEligibleIndexColumns(table);

        // Next, calculate IO cost of each index scan, if any,
        // and select cheapest
        QueryOperator indexOp;
        int indexCost;
        int exceptIndex = -1;
        for (Integer i : selectWithIndex) {
            indexOp = new IndexScanOperator(transaction, table, selectColumnNames.get(i),
                    selectOperators.get(i), selectDataBoxes.get(i));
            indexCost = indexOp.estimateIOCost();
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        Map<Set, QueryOperator> lastPass = new HashMap<>();
        QueryOperator minCostJoinOp = null;

        // TODO(proj3_part2): implement
        //joinTableNames;
        //joinLeftColumnNames;
        //joinRightColumnNames;
        ...
        for (Set tableSet : prevMap.keySet()) {
            for (int i = 0; i < joinTableNames.size(); ++i) {
                ...
                // set variables for left table and access operator
                ...
                // set variables for right table and access operator
                ...
                // Case 1: contains left table, but not right table
                if (tableSet.contains(leftTable) && !tableSet.contains(rightTable)) {
                    newSet = new HashSet<>(Collections.singleton(rightTable));
                    leftOp = prevMap.get(tableSet);
                    rightOp = pass1Map.get(newSet);
                }
                // Case 2: contains right table, but not left table
                else if (tableSet.contains(rightTable) && !tableSet.contains(leftTable)) {
                    newSet = new HashSet<>(Collections.singleton(leftTable));
                    leftOp = pass1Map.get(newSet);
                    rightOp = prevMap.get(tableSet);
                }
                // Case 3: if neither or both sides in tableSet
                else {
                    continue;
                }

                minCostJoinOp = minCostJoinType(leftOp, rightOp, leftCol, rightCol);
                // table, table
                newSet.addAll(tableSet);
                if (!lastPass.containsKey(newSet)) {
                    lastPass.put(newSet, minCostJoinOp);
                }
                // add if this order of join is cheaper
                else if (minCostJoinOp.getIOCost() < lastPass.get(newSet).getIOCost()) {
                    lastPass.put(newSet, minCostJoinOp);
                }
            }
        }
