...
def balanced(m):
    ...
    if is_weight(m):
        ...
        return (balanced(end(left(m)))) and balanced(end(right(m))) and t1 == t2
...
def replace_leaf(t, old, new):
    ...
    if label(t) == old and is_leaf(t):
        ...
        sub_trees = [replace_leaf(b, old, new) for b in branches(t)]
...
def make_fib():
    ...
    next_num = 1
    def next_fibo():
        nonlocal start_num, next_num
        curr_num, start_num = start_num, next_num
        next_num += curr_num
        return curr_num
    return next_fibo
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    wrong_passwords = []
    def withdraw(withdraw_amount, password_given):
        nonlocal wrong_passwords, password
        if len(wrong_passwords) == 3:
            return f"Your account is locked. Attempts: ['{wrong_passwords[0]}', '{wrong_passwords[1]}', '{wrong_passwords[2]}']"

        if password_given != password:
            wrong_passwords.append(password_given)
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    error = withdraw(0, old_password) # check if account functions
    if type(error) == str:
        return error

    def joint(amount, attempt):
        if attempt == new_password: # check if new password, then correct
