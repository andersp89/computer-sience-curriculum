...
class LeafNode extends BPlusNode {
	...
	public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
		// TODO(proj2): implement
		if (keys.contains(key)) throw new BPlusTreeException("Duplicate keys are not allowed!");
		
		insertKeyAndRidInArrays(key, rid);
		int d = metadata.getOrder();
		// case 1: no overflow at leaf
		if (keys.size() <= (d * 2)) {	
			sync();
			return Optional.empty();
		} 
		// case 2: overflow at leaf
		else {
			// keep d entries in this leaf and move d+1 entries to new
			...
			return createNewNodeAndSetRightSiblings(keys, rids);
		}
	}
	...
	public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
			float fillFactor) {
		// TODO(proj2): implement
		// fill up to 1 record more than fillFactor 
		// then split by creating a right sibling 
		int d = metadata.getOrder();
		int fFactor = (int) Math.ceil(d * 2 * fillFactor);
		
		while (data.hasNext()) {
			Pair<DataBox, RecordId> nextElement = data.next();
			keys.add(nextElement.getFirst());
			rids.add(nextElement.getSecond());
			// fill up to one record more than fillFactor
			if (keys.size() == fFactor + 1) {
				// split by creating a right sibling
				List<DataBox> keys = new ArrayList<>();
				List<RecordId> rids = new ArrayList<>();	
				keys.add(this.keys.remove(fFactor));	
				rids.add(this.rids.remove(fFactor));
				return createNewNodeAndSetRightSiblings(keys, rids);	
			}
		}
		sync();
	...
	// Helpers ///////////////////////////////////////////////////////////////////
	private void insertKeyAndRidInArrays(DataBox key, RecordId rid) {
		int i = 0;
		for (DataBox keyInLeaf : keys) {
			if (key.compareTo(keyInLeaf) > 0) 
				++i;
			else
				break;
		}
	...
	private Optional<Pair<DataBox, Long>> createNewNodeAndSetRightSiblings(List<DataBox> keys, List<RecordId> rids) {
		// set right sibling of sibling leaf
		Optional<Long> rightSibling = Optional.empty();
		if (this.rightSibling.isPresent()) {
			rightSibling = this.rightSibling; 
		}
		
		// create new sibling and set rightsibling of this leaf
		LeafNode siblingLeaf = new LeafNode(metadata, bufferManager, keys, rids, rightSibling, treeContext);
		Long pageNumOfSibling = siblingLeaf.page.getPageNum();
		this.rightSibling = Optional.of(pageNumOfSibling);
		
		// return split key and page number of newly created node
		sync();
		return Optional.of(new Pair<DataBox, Long>(siblingLeaf.keys.get(0), pageNumOfSibling));
	...
	public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
		...
		// check that page is a LeafNode
		assert (buf.get() == (byte) 1);
		
		Long rightSiblingObject = buf.getLong();
		Optional<Long> rightSibling = (rightSiblingObject == (byte) -1) ? Optional.empty() : Optional.of(rightSiblingObject);
