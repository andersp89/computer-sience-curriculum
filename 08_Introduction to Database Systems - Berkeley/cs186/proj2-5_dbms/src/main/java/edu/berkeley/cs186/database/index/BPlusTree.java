...
public class BPlusTree {
	...
	public Optional<RecordId> get(DataBox key) {
		...
		LeafNode nodePossiblyWithKey = root.get(key);
		return nodePossiblyWithKey.getKey(key);
	}
	...
	public Iterator<RecordId> scanGreaterEqual(DataBox key) {
		...
		return new BPlusTreeIterator(startNode, Optional.of(key));
	}
	...
	public void put(DataBox key, RecordId rid) {
		...
		if (response.isPresent()) {
			createAndUpdateNewRootFrom(response.get());
		}
	}
	...
	public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
		...
		// TODO(proj4_part3): B+ tree locking
		LockUtil.ensureSufficientLockHeld(lockContext, LockType.S);
		if (scanAll().hasNext()) throw new BPlusTreeException("Tree must be empty!");
		while (data.hasNext()) {
	...
	// Helpers /////////////////////////////////////////////////////////////////
	private void createAndUpdateNewRootFrom(Pair<DataBox, Long> response) {
		List<DataBox> keys = new ArrayList<>();
		keys.add(response.getFirst());
		List<Long> children = new ArrayList<>();
		children.add(root.getPage().getPageNum());
		children.add(response.getSecond());
	...
	private class BPlusTreeIterator implements Iterator<RecordId> {
		...
		private Iterator<RecordId> curRidsIter;
		
		public BPlusTreeIterator(BPlusNode startNode, Optional<DataBox> key) {
			curLeaf = startNode.getLeftmostLeaf();
			if (key.isPresent()) {
				// scanGreaterEqual once, thereafter all keys,
				// in remaining nodes are greater than key
				curRidsIter = curLeaf.scanGreaterEqual(key.get());
			} else {
				curRidsIter = curLeaf.scanAll();
			}
		}
		...
		public boolean hasNext() {
			// TODO(proj2): implement
			if (this.curRidsIter.hasNext()) {
				return true;
			}
			
			// iterate over sibling, if any
			Optional<LeafNode> rightSibling = checkIfRightSiblingAndSetLeafAndIter();
		...
		public RecordId next() {
			// TODO(proj2): implement			
			if (curRidsIter.hasNext()) {
				return curRidsIter.next();
			}
		...
		// Helper methods
		private Optional<LeafNode> checkIfRightSiblingAndSetLeafAndIter() {
