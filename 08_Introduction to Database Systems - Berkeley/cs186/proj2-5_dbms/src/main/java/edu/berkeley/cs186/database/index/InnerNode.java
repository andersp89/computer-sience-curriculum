...
class InnerNode extends BPlusNode {
	...
	public LeafNode get(DataBox key) {
		...
		int childToVisit = numLessThanEqual(key, keys);
		BPlusNode nextNode = getChild(childToVisit);
	...
	public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
		// TODO(proj2): implement
		// insert in child LeafNode
		int childToVisit = numLessThanEqual(key, keys);
		BPlusNode nextNode = getChild(childToVisit);
		// USE GET METHOD INSTEAD?
		Optional<Pair<DataBox, Long>> responseFromLeaf = nextNode.put(key, rid);

		// no split at leaf
		if (responseFromLeaf.equals(Optional.empty())) {
			return Optional.empty();
		} 

		// split at leaf, hence insert in inner
		DataBox newKey = responseFromLeaf.get().getFirst();
		Long pageNumOfNewLeaf = responseFromLeaf.get().getSecond();
		insertKeyAndChildInArrays(newKey, pageNumOfNewLeaf);
		int d = metadata.getOrder();
		// case 1: no overflow at inner
		if (keys.size() <= d * 2) { 
			sync();
			return Optional.empty();
		}
		// case 2: overflow at inner
		else {
			return splitEntriesBetweenOrigInnerAndNewInner();
		}
	}
	...
	public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
		...
		while (data.hasNext()) {
			// TODO(proj2): implement
			// insert in child LeafNode
			int childToVisit = children.size() - 1;
			BPlusNode nextNode = getChild(childToVisit);
			Optional<Pair<DataBox, Long>> responseFromLeaf = nextNode.bulkLoad(data, fillFactor);
	...
	// Helpers ///////////////////////////////////////////////////////////////////
	private void insertKeyAndChildInArrays(DataBox key, Long child) {
		int i = 0;
		for (DataBox keyInInner : keys) {
			if (key.compareTo(keyInInner) > 0) 
				++i;
			else
				break;
		}
	...
	private Optional<Pair<DataBox, Long>> splitEntriesBetweenOrigInnerAndNewInner() {
		// keep first d entries in this inner, and 
		// move last d entries to new inner
		ArrayList<DataBox> keys = new ArrayList<>();
		ArrayList<Long> children = new ArrayList<>();
		int d = metadata.getOrder();
		
		// middle entry is moved up as split key
		DataBox splitKey = this.keys.remove(d);
		...
		// return split key and page number of new inner node
		InnerNode newInnerNode = new InnerNode(metadata, bufferManager, keys, children, treeContext);
		Long pageNumOfNewInner = newInnerNode.page.getPageNum();
		sync();
		//Pair<DataBox, Long> splitKeyAndPageNumOfNewInner = new Pair<>(splitKey, pageNumOfNewInner); //new pair in return statement!
		return Optional.of(new Pair<DataBox, Long>(splitKey, pageNumOfNewInner));
	}
