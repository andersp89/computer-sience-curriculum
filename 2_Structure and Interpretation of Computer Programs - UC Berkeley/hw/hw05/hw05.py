...
def make_counter():
    ...
    def counter(s):
        ...
        if s not in string_count:
            ...
            string_count[s] = string_count[s] + 1
        return string_count[s]
    return counter
...
class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.deposited_amount < self.price:
            return 'You must deposit ${0} more.'.format(self.price - self.deposited_amount)
        elif self.deposited_amount > self.price:
            change = self.deposited_amount - self.price
            self.deposited_amount = 0
            self.stock -= 1
...
def preorder(t):
    ...
    """if t.is_leaf():
        ...
        all_notes = []
        all_notes.extend([t.label])
        for b in t.branches:
            all_notes.extend(preorder(b))
        return all_notes"""
    lst = []
    def preorderHelper(t, lst):
        lst.append(t.label)
        for b in t.branches:
            preorderHelper(b, lst)
    preorderHelper(t, lst)
    return lst
