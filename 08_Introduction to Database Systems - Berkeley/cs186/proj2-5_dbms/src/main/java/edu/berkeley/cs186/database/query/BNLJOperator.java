...
class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator extends JoinIterator {
        ...
        private void fetchNextLeftBlock() {
            ...
            // leftRecordIterator: iterator over B-2 pages that have record in them
            // leftRecrod: first record in this block
            ...
            int usableBuffers = numBuffers - 2;
            leftRecordIterator = getBlockIterator(this.getLeftTableName(), leftIterator, usableBuffers);
        ...
        private void fetchNextRecord() {
            ...
            nextRecord = null;
            while (!hasNext()) {
                // for each left record, test all right records
                ...
                // fetch next left record in block and test with right records in page
                else if (leftRecordIterator.hasNext()) {
                    fetchNextLeftRecordAndResetRightPage();
                }
                // fetch next right page and test for all left records in block
                else if (rightIterator.hasNext()) {
                    fetchNextRightPage();
                    resetLeftRecord();
                }
                // fetch next left block to test and reset right side
                else if (leftIterator.hasNext()) {
                    fetchNextLeftBlock();
                    rightIterator.reset();
                    fetchNextRightPage();
                }
                // no more left blocks to test
                else {
                    return;
                }
            }
        ...
        private void fetchNextLeftRecordAndResetRightPage() {
