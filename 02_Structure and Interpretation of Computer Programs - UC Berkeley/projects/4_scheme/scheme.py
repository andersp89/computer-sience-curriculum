...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda x: scheme_eval(x, env)) # apply scheme_eval on all operands (arguments)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) is not len(vals):
            raise SchemeError("Number of parameters do not match number of values")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Not correct number of arguments provided')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        expression = LambdaProcedure(expressions.first.second, expressions.second, env)
        env.define(target.first, expression)
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    elif expressions.second is nil: # return the last value
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            # if no sub-expression, return predicate value
...
def make_let_frame(bindings, env):
    ...
    #print(bindings) # all bindings ((x 5))

    #print(bindings.first) # (x 5)
    #print(bindings.first.first) # x, for parameter
    #print(bindings.first.second.first) # 5, for argument
    symbols, args = nil, nil
